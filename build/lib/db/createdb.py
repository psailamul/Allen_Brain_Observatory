...
def get_all_pointers(config, cid,sess,stim):
  ...
  if stim in config.available_stims:
    stim_template="%s%s_template.pkl"%(config.stimulus_template_loc,stim)
  else:
    stim_template=None
  pointer_list ={
    'fluorescence_trace':"%s%s_%s_traces.npz"%(config.fluorescence_traces_loc,cid,sess_type),
    'stim_table':"%s%s_%s_%s_table.npz"%(config.stim_table_loc,sess['id'],sess_type,stim),
    'other_recording':"%s%s_related_traces.npz"%(config.specimen_recording_loc,sess['id']),
    'ROImask':"%s%s_%s_ROImask.npz"%(config.ROIs_mask_loc,cid,sess_type),
