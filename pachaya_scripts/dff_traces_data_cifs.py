...
def save_dff_neural_traces(data_set, common_cell_id, sess,
                                                            save_loc=config.dff_traces_loc):
    # Save the corrected_fluorescence traces, running speed, motion correction and eye position
    import numpy as np
    sess_type=sess['session_type']
    metadata = sess.copy()
    ...
    #tstamp, corrected_traces = data_set.get_corrected_fluorescence_traces(cell_specimen_ids=sort_id_by_indices) 
    tstamp, dff_traces = data_set.get_dff_traces(cell_specimen_ids=sort_id_by_indices) 
    ...
    for i, cid in tqdm(
        ...
        fname ="%s%s_%s_dff_traces.npz"%(save_loc,cid,sess_type)
        np.savez(fname,
            tstamp=tstamp, 
            dff_trace=dff_trace,
            epoch_table = epoch_table, 
...
for idx in tqdm(
    ...
    for sess in exp_session:
        ...
        traces_loc_pointer= save_dff_neural_traces(# save neural traces for all cell in this session [cell-wise]
                                                            data_set=data_set,
